...
%\newcommand{\VOTableSpec}  {TAP specification \citet{std:VOTable}\xspace}
...
%\newcommand{\TAPRegSpec}{TAPRegExt specification \citep{std:TAPREGEXT}\xspace}

\usepackage{etoolbox}
\newcommand{\IvoaCite}[3]{\providebool{#1}\ifbool{#1}{#3\xspace}{#3\xspace(\citet{#2})\booltrue{#1}}\xspace}
...
\begin{itemize}
    ...
    \item 20171129-008 INTERVAL is define in DALI but not in ADQL. \SectionSee{sec:types.numeric.interval}
