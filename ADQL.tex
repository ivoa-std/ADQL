            ...
            \item \textbf{Added}
                \begin{itemize}
                    ...
                    \item Description of accepted datatypes and their mapping
                          with VOTable \SectionSee{sec:types}
                    \item Language feature description \SectionSee{sec:features}
                \end{itemize}
            \item \textbf{Updated}
                \begin{itemize}
...
\label{sec:changes-future}

\begin{itemize}
    \item Move from BNF to PEG grammar
    \item Filtering on timestamp values
    \item Support interval values
    \item Support MOC
    \item Support array values
    \item Support bitwise operators
    \item Support hexadecimal values
    \item Better support of boolean values (i.e. boolean type ; no more
          \verb:=1:)
    \item Add a substring function (already existing in SQL-92)
    \item Extend \verb:DISTANCE(): to other geometries (not only
          \verb:POINT():s)
    \item Add a function returning the polygonal intersection of two geometries:
          \verb:INTERSECTION():
    \item Unit annotation of values
    \item Consider the OGC Standard (OpenGIS\textsuperscript{\textregistered}
          Implementation Standard for Geographic information) for ADQL
