...
ADQL also provides support for STC-S based geometic regions,
...
To fully support the REGION datatype, a service implementation SHOULD
provide implementations of the INTERSECTS and CONTAINS operators that
support the REGION datatype in combination with the other STC geometric
...
\subsubsection{REGION}
 ...
 This function is a literal constructor for the datatype REGION. It provides a
 way of expressing a complex region represented by a single string input
 parameter. It MUST return a value with the datatype REGION as introduced in
 \SectionRef{sec:types.geom.region}. Consequently, the format of its string
 parameter MUST follow the \STCSSpec.

 For example:

 \begin{verbatim}
 -- STC-s - Complex region
 REGION('Union ICRS TOPOCENTER
