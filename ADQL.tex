...
ADQL also provides support for a text based description of a
geomertic region using the REGION datatype. 
...
cannot be expressed by one of the simple geometric types described in
the previous sections.

REGION values are serialized as character arrays with the xtype of
\verb:region: that contain a STC-S string serialization describing the
geometric region.
The grammar for the content of the STC-S string is defined in the
\STCSspec.

If a service implementation supports the REGION datatype, then it SHOULD
provide implementations of the INTERSECTS and CONTAINS operators that
support combinations of the REGION datatype with the other geometric
