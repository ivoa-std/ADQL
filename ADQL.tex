...
%and POLYGON) require a string-valued first argument. It was intended to carry
...
\end{verbatim}
\noindent
%TODO - speculative, not definative.
...
If the two geometric arguments are expressed in different coordinate systems,
the function is responsible for converting one (or both).
...
If the arguments are expressed in different coordinate systems, the
INTERSECTS function is responsible for converting one, or both, of the
arguments into a different coordinate system.
If it cannot perform the required conversion then it SHOULD throw an
error. Details of the error message are defined by the service
executing the query.
