...
In addition, some service implementations may also support for the optional
ILIKE case-insensitive string comparison operator.
...
(\citet{std:VODS11}) or TAP's \verb:TAP_SCHEMA: (\citet{std:TAP}).
...
The \citet{std:TAPREGEXT} standard defines an XML schema that a service SHOULD
...
The list of ADQL reserved keywords shown in Section \ref{sec:keywords}
is therefore extended with the following function names:
...
or POLYGON), all the required numeric arguments, and, in this version,
a blank coordinate system string. For example, the query
...
is defined as part of the \cite{std:TAPREGEXT} standard.
...
See the \citet{std:TAPREGEXT} standard for full details on how to use the
...
The list of ADQL reserved keywords shown in Section \ref{sec:keywords}
is therefore extended to include the following:
...
The LOWER function converts its string paremeter to lower case, using
the standard mapping for ASCII characters, and algorithm R2 described in
section 3.13, "Default Case Algorithms" of \citet{std:UNICODE} for characters
outside ASCII.
...
\subsection{Hierarchical queries}
\label{sec:hierarchical}

An ADQL service implementation MAY include support for the following optional
support for hierarchical or recursive queries :
...
\subsubsection{WITH}
{\footnotesize Language feature :}\\
{\footnotesize \verb|type: ivo://ivoa.net/std/TAPRegExt#features-adql-hierarchical|}\\
...
The BNF sytntax for a recursive \verb:WITH: query is
\begin{verbatim}
    <recursive_query> :=
        WITH <with_query> [, ...]
        <select_query>
...
description using the formatting defined in \citet{std:VOUNIT}.
...
An ADQL service implementation MAY include support for the following optional
bitwise comparison functions, and their corresponding operators:

\begin{itemize}
    \item \verb:BIT_AND(x,y)  x & y: 
    \item \verb:BIT_OR(x,y)   x | y:
    \item \verb:BIT_XOR(x,y)  x ^ y:
    \item \verb:BIT_NOT(x)    ~ x:
\end{itemize}

The list of ADQL special characters shown in Section \ref{sec:characters}
is therefore extended to include the following:

\begin{itemize}
    \item curcumflex (\verb:^:)
    \item tilde (\verb:~:)
\end{itemize}
...
The function and operator form of each bitwise operation are treated as a
pair. If an ADQL implementation declares support for one of the bitwise
...
The \verb:BIT_AND(): function and the correspoding ampersand operator
\verb:'&': perform a bit wise AND operation on two integer operands.
...
The \verb:BIT_OR(): function and the correspoding 'vertical bar' operator
\verb:'|': perform a bit wise OR operation on two integer operands.
...
The \verb:BIT_XOR(): function and the correspoding curcumflex operator
\verb:'^': perform a bit wise XOR (exclusive or) operation on two integer
operands.
...
The \verb:BIT_NOT(): function and the correspoding tilde operator \verb:'~':
perform a bit wise NOT operation on an integer operand.
...
Within the \citet{std:TAPREGEXT} XML schema, each group of features is
