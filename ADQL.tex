...
When function arguments for a geometric function represent
spherical coordinates, the units MUST be in degrees
(square degrees for area).

When function arguments for a geometric function represent
cartesian coordinates, the vector coordinates MUST be normalized.
...
either a POINT function (See \ref{sec:functions.geom.point}) or
...
\begin{itemize}
    ...
    \item the center position is a comma separated numeric duple, with units and legal ranges as defined in Section \ref{sec:functions.geom.overview}.
...
extracted from either a POINT function (See \ref{sec:functions.geom.point})
...
corresponds semantically to the STC Circle. The
...
POINT function (See \ref{sec:functions.geom.point}) or the coordinate’s column references:
