...
The following terms are ADQL keywords that are no longer used,
but still remain reserved for backwards compatibility.
...
\begin{table}[thm]\footnotesize
    \begin{tabular}{|p{0.20\textwidth}|p{0.125\textwidth}|p{0.125\textwidth}|p{0.55\textwidth}|}
        ...
        \textit{y} double &
        double &
        Returns the remainder of \textit{x/y} as a double.
        \tabularnewline
...
The results of an ADQL query may return a complex region
that cannot be expressed by one of the simple geometric types
defined in the preceding sections.

In order to support complex geometric regions,
the results of an ADQL query may use the xtype \verb:region:
to describe a column that contains a STC-S string
serialization.

The grammar for the content of the STC-S string is defined in the \STCSspec.
...
contains geometric (POINT, BOX, CIRCLE or POLYGON) values.
...
database columns that contain POINT and geometric (BOX, CIRCLE or POLYGON)
values respectively.
%TODO - ObsCore example
...
database columns that contain geometric (BOX, CIRCLE or POLYGON) values.
...
This behaviour will be dropped in the next major version of this specification.
...
The arguments specify the position, where:
\begin{itemize}
    \item the two numeric coordinates match the units and ranges defined in \SectionRef{sec:functions.geom.overview}.
\end{itemize}
...
\begin{itemize}
    \item the coordinates are given as pairs of numeric values,
    with units and legal ranges as defined in
    \SectionRef{sec:functions.geom.overview}.
\end{itemize}
...
The coordinates for the POLYGON may be literal values, as above,
or they may be column references, functions or expressions that return
numeric values.
For example:
\begin{verbatim}
...
\end{verbatim}
\noindent
where \textit{t.ra} and \textit{t.dec} are references to database columns
that contain numeric values.
%TODO - ObsCore example

An alternative form of the POLYGON function accepts a series of POINT values
%TBD ..
